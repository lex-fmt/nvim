-- Test: LSP toggle annotations command
...
local exe = vim.fn.exepath("lexd-lsp"); local lex_lsp_path = vim.env.LEX_LSP_PATH or (exe ~= "" and exe) or (project_root .. "/target/debug/lexd-lsp")
...
-- Test 1: Toggle annotations command returns a workspace edit when on an annotation
...
if not result then
  print("TEST_FAILED: No toggle_annotations result at all")
  vim.cmd("cquit 1")
end

if result.err then
  print("TEST_FAILED: toggle_annotations error: " .. vim.inspect(result.err))
  vim.cmd("cquit 1")
end
...
if result.result then
  ...
  if not edit.changes and not edit.documentChanges then
    print("TEST_FAILED: toggle_annotations response is not a valid workspace edit")
    vim.cmd("cquit 1")
  end
  print("TEST_PASSED: toggle_annotations returns valid workspace edit")
else
  -- No edit returned - might mean no annotation at position
  print("TEST_PASSED: toggle_annotations returns nil (no annotation at position)")
end
...
if type(commands.toggle_annotations) ~= "function" then
  print("TEST_FAILED: commands.toggle_annotations is not a function")
  vim.cmd("cquit 1")
end

print("TEST_PASSED: commands module exports toggle_annotations function")
print("TEST_PASSED: All toggle annotations tests passed")
vim.cmd("qall!")
