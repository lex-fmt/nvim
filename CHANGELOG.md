...
- Lex Monochrome theme colors are now resolved at generate time, matching the strategy used by the vscode and zed editor packages. `app-bin/gen-theme.py` emits `lua/lex/theme_data.lua` with per-mode `PALETTE` and `RULES` tables carrying absolute hex values pre-resolved from the canonical intensity/background tiers in `comms/shared/theming/lex-theme.json`. The `apply_monochrome` runtime path picks the table by `vim.o.background` and applies entries directly — no `colors[rule.intensity]` indirection. No visual change; same hex values, resolved once at generate time instead of at every apply. (#26, #27)
